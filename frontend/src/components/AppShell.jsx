...
export default function AppShell({ title, children, unreadCount = 0 }) {
  ...
  return <div className="app-shell">
    ...
    {profileOpen && <div className="modal-backdrop" role="presentation"><section className="profile-modal" role="dialog" aria-modal="true" aria-labelledby="profile-title"><div className="panel-heading"><div><p className="eyebrow">Account</p><h2 id="profile-title">My profile</h2></div><button className="icon-button" onClick={closeProfile} aria-label="Close profile">×</button></div><dl><div><dt>Name</dt><dd>{email.split("@")[0]}</dd></div><div><dt>Email</dt><dd>{email}</dd></div><div><dt>Role</dt><dd>{role}</dd></div></dl><p className="helper-text">Profile details are managed by HR.</p></section></div>}
  </div>;
}
