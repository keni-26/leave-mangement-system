...
function EmployeeDashboard() {
  ...
  return (
    <main className="employee-dashboard">
      <header className="dashboard-header"><div><h1>Employee Dashboard</h1><p>{email || "Signed-in employee"}</p></div><button type="button" className="secondary-button" onClick={clearAuthentication}>Logout</button></header>
      ...
      <section className="dashboard-section"><h2>Leave Balance</h2>{balances.length === 0 ? <p className="empty-state">No leave balances are available yet.</p> : <div className="balance-grid">{balances.map((balance) => <article className="balance-card" key={balance.id}><h3>{balance.leaveType?.name || "Leave Type"}</h3><dl><div><dt>Allocated</dt><dd>{balance.allocatedDays ?? 0}</dd></div><div><dt>Used</dt><dd>{balance.usedDays ?? 0}</dd></div><div><dt>Remaining</dt><dd>{balance.remainingDays ?? 0}</dd></div></dl></article>)}</div>}</section>

      <section className="dashboard-section"><h2>Available Leave Types</h2>{leaveTypes.length === 0 ? <p className="empty-state">No active leave types are available.</p> : <div className="leave-type-grid">{leaveTypes.map((leaveType) => <article className="leave-type-card" key={leaveType.id}><h3>{leaveType.name}</h3><p>{leaveType.description || "No description provided."}</p><p><strong>Allocated:</strong> {leaveType.allocatedDays ?? 0} days</p><p><strong>Approval:</strong> {leaveType.approvalRequired ? "Manager approval required" : "Auto-approved"}</p></article>)}</div>}</section>

      <section className="dashboard-section"><h2>Apply for Leave</h2><form className="leave-form" onSubmit={handleApplyLeave}><label>Leave type<select name="leaveTypeId" value={form.leaveTypeId} onChange={handleFormChange} required><option value="">Select a leave type</option>{leaveTypes.map((leaveType) => <option key={leaveType.id} value={leaveType.id}>{leaveType.name}</option>)}</select></label><label>Start date<input type="date" name="startDate" value={form.startDate} onChange={handleFormChange} required /></label><label>End date<input type="date" name="endDate" value={form.endDate} onChange={handleFormChange} required /></label><label className="form-full-width">Reason<textarea name="reason" value={form.reason} onChange={handleFormChange} rows="4" required /></label><button type="submit" disabled={submitting}>{submitting ? "Submitting..." : "Submit Leave Request"}</button></form></section>

      <section className="dashboard-section"><h2>My Leave Requests</h2>{leaveRequests.length === 0 ? <p className="empty-state">You have not submitted any leave requests.</p> : <div className="table-wrapper"><table><thead><tr><th>Leave type</th><th>Dates</th><th>Days</th><th>Reason</th><th>Status</th><th>Created</th><th>Action</th></tr></thead><tbody>{leaveRequests.map((request) => <tr key={request.id}><td>{request.leaveType?.name || "-"}</td><td>{formatDate(request.startDate)} – {formatDate(request.endDate)}</td><td>{request.leaveDays ?? 0}</td><td>{request.reason}{request.rejectionReason && <small className="rejection-reason">Rejection reason: {request.rejectionReason}</small>}</td><td><span className={`status status-${String(request.status).toLowerCase()}`}>{request.status}</span></td><td>{formatDate(request.createdAt)}</td><td>{request.status === "PENDING" && <button type="button" className="danger-button" disabled={updatingId === `leave-${request.id}`} onClick={() => handleCancelLeave(request.id)}>{updatingId === `leave-${request.id}` ? "Cancelling..." : "Cancel"}</button>}</td></tr>)}</tbody></table></div>}</section>
      ...
      <section className="dashboard-section"><h2>Upcoming Holidays</h2>{upcomingHolidays.length === 0 ? <p className="empty-state">No upcoming holidays are listed.</p> : <div className="holiday-list">{upcomingHolidays.map((holiday) => <article key={holiday.id}><h3>{holiday.name}</h3><p>{formatDate(holiday.holidayDate)}</p><p>{holiday.description || "No description provided."}</p></article>)}</div>}</section>
    </main>
  );
}
